// Vocabulario por temas. Cada entrada: es (español), en (traducción), tipo opcional "frase".
window.DATA_VOCABULARIO = {
  "temas": {
    "futbol-basico": [
      {"es": "el balón", "en": "the ball"},
      {"es": "el pase", "en": "the pass"},
      {"es": "el gol", "en": "the goal (score)"},
      {"es": "la portería", "en": "the goal (net)"},
      {"es": "el equipo", "en": "the team"},
      {"es": "el campo", "en": "the field"},
      {"es": "¡pásala!", "en": "pass it!", "tipo": "frase"},
      {"es": "¡gooool!", "en": "goooal!", "tipo": "frase"}
    ],
    "posiciones": [
      {"es": "la izquierda", "en": "the left"},
      {"es": "la derecha", "en": "the right"},
      {"es": "arriba", "en": "up"},
      {"es": "abajo", "en": "down"},
      {"es": "cerca", "en": "near"},
      {"es": "lejos", "en": "far"},
      {"es": "adelante", "en": "forward"},
      {"es": "atrás", "en": "back"}
    ],
    "numeros": [
      {"es": "uno", "en": "one"},
      {"es": "dos", "en": "two"},
      {"es": "tres", "en": "three"},
      {"es": "cuatro", "en": "four"},
      {"es": "cinco", "en": "five"},
      {"es": "seis", "en": "six"},
      {"es": "siete", "en": "seven"},
      {"es": "ocho", "en": "eight"},
      {"es": "nueve", "en": "nine"},
      {"es": "diez", "en": "ten"},
      {"es": "pasa al número siete", "en": "pass to number seven", "tipo": "frase"}
    ],
    "jugadores": [
      {"es": "el portero", "en": "the goalkeeper"},
      {"es": "el defensa", "en": "the defender"},
      {"es": "el centrocampista", "en": "the midfielder"},
      {"es": "el delantero", "en": "the striker"},
      {"es": "el capitán", "en": "the captain"},
      {"es": "el árbitro", "en": "the referee"},
      {"es": "el entrenador", "en": "the coach"}
    ],
    "frases": [
      {"es": "el delantero está desmarcado", "en": "the striker is open", "tipo": "frase"},
      {"es": "pasa el balón al portero", "en": "pass the ball to the goalkeeper", "tipo": "frase"},
      {"es": "cuidado con el defensa", "en": "watch out for the defender", "tipo": "frase"},
      {"es": "un pase largo a la derecha", "en": "a long pass to the right", "tipo": "frase"},
      {"es": "tenemos poco tiempo", "en": "we have little time", "tipo": "frase"},
      {"es": "¡qué buena jugada!", "en": "what a great play!", "tipo": "frase"}
    ]
  }
};
